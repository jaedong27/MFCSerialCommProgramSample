...
BOOL CCommThread::OpenPort(CString strPortName, DWORD dwBaud, HWND hWnd )
{
	return OpenPort(strPortName, dwBaud, 8, 0, 0, hWnd);
}
...
BOOL CCommThread::OpenPort(CString strPortName, 
					   DWORD dwBaud, BYTE byData, BYTE byStop, BYTE byParity, HWND hWnd )
{
	...
	// Local 변수.
		...
		// hWnd 에 이벤트 핸들러 전달할 다이얼로그 핸들값을 넣어줌
		m_hCommWnd = hWnd;
	...
	/*m_hThreadWatchComm = CreateThread( NULL, 0, 
									   ...
									   this, 0, &dwThreadID);*/
	CWinThread* pThread = AfxBeginThread((AFX_THREADPROC)ThreadWatchComm, (LPVOID)this);
...
UINT CCommThread::ThreadWatchComm(CCommThread* pComm)
{
   DWORD           dwEvent;
   OVERLAPPED      os;
   BOOL            bOk = TRUE;
   ...
   while (pComm ->m_bConnected)//포트가 연결되면 무한 루프에 들어감
   {
        ...
        if ((dwEvent & EV_RXCHAR) == EV_RXCHAR)
        {
			...
			::PostMessage(pComm->m_hCommWnd, WM_COMM_READ, 0, 0 );//CSerialComDlg로 데이터가 들어왔다는 메시지를 보냄
		}
