...
public:
	...
	//--------- 외부 사용 함수 ------------------------------------//
	BOOL    OpenPort(CString strPortName, DWORD dwBaud, HWND hWnd );
	BOOL	OpenPort(CString strPortName, 
					   DWORD dwBaud, BYTE byData, BYTE byStop, BYTE byParity, HWND hWnd );//포트 열기 
	...
	// Thread로 사용할 함수 
	UINT	static ThreadWatchComm(CCommThread* pComm);
	HWND    m_hCommWnd;
};
